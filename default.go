...
type Store interface {
	...
	GetAll(count int, resource string) ([][][]byte, [][]byte, error)
	GetAllAfter(key []byte, count int, resource string) ([][][]byte, [][]byte, error)  //Get all items after a key
	GetAllBefore(key []byte, count int, resource string) ([][][]byte, [][]byte, error) //Get all items before a key
	Filter(prefix []byte, count int, resource string) ([][][]byte, [][]byte, error)
